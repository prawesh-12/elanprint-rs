#!/bin/sh
# Install elanprintd as the system fingerprint daemon. Run with sudo.
#
...
[ -f target/release/elanprintd ] || {
  echo "build first: cargo build --release -p elanprintd"
  exit 1
}

echo "1. binary -> /usr/libexec/elanprintd"
install -D -m 0755 target/release/elanprintd /usr/libexec/elanprintd

echo "2. store -> /var/lib/elanprint/prints.json"
install -d -m 0700 /var/lib/elanprint
if [ -f /var/lib/elanprint/prints.json ]; then
  echo "   keeping the store already there"
else
  SRC="${ELANPRINT_SEED:-/tmp/elanprint-prints.json}"
  [ -f "$SRC" ] || { echo "   no seed store at $SRC"; exit 1; }
  install -m 0600 "$SRC" /var/lib/elanprint/prints.json
  echo "   seeded from $SRC"
fi
cat /var/lib/elanprint/prints.json

echo "3. unit -> /etc/systemd/system/elanprintd.service"
...
sleep 2
systemctl --no-pager --lines=5 status elanprintd.service || true
