...
//!
//! The daemon runs as root on the system bus, and the shipped policy in
//! `/usr/share/dbus-1/system.d/net.reactivated.Fprint.conf` lets any local
//! user send to the Device interface. It can do that safely for the stock
//! fprintd because fprintd checks polkit. This daemon does not use polkit,
//! so the check lives here: root, or the owner of the prints, nobody else.
//!
//! Root is allowed because `gdm-session-worker` runs as root while
//! `pam_fprintd` authenticates, which is the entire greeter login path.
//! See D-031.

use std::path::Path;

/// Where account names map to uids.
pub const PASSWD: &str = "/etc/passwd";

/// uid of `user`, parsed from the contents of a passwd file.
///
/// Pure so the parsing is testable. Fields are
...
///
/// An unknown target user is refused for everyone but root, so a typo or a
/// deleted account can never widen access.
...
/// uid of `user` on this machine, or `None` if there is no such account.
...
";
    ...
    /// `DeleteEnrolledFingers` is the call that erased the slot 0 template.
    ///
    /// It is the one method where a wrong answer costs flash with no undo, so
    /// it gets the case-by-case table rather than a spot check.
    #[test]
