...
use elanprint_proto::SlotState;

/// Mapping file, mode 0600, owned by root.
pub const DEFAULT_PATH: &str = "/var/lib/elanprint/prints.json";
...
impl Store {
    /// Starts empty when the file does not exist.
    ...
    /// Names are validated and slots stay unique.
...
///
/// Slots below `count` (`enrolled_num`) are presumed occupied: on 0c90 an
/// occupied slot answers `finger_info` with the same `40 ff` an empty one
/// does, so no scan separates them. The store only removes further
/// candidates, never restores one, so it can never hand out a live slot.
...
///
/// The 2 byte `40 ff` form proves nothing: an invalid id and an empty slot
/// answer identically. An entry is `stale` only when the device proves it,
/// by `enrolled_num` at 0 or a 70 byte record.
...
mod tests {
    ...
    fn tmp_path(tag: &str) -> PathBuf {
        ...
        p.push(format!("elanprint-store-{}-{}.json", std::process::id(), tag));
        p
    }
    ...
    #[test]
    fn empty_store_never_reopens_a_counted_slot() {
