//! elanprintd: fprintd-compatible daemon for the ELAN 04f3:0c90.
//!
//! Owns `net.reactivated.Fprint` on the system bus, implementing
//! `docs/dbus-device.xml`. One USB claim, armed once, behind a mutex: a
//! second client gets `AlreadyInUse`.
...
/// Tests drive a fake transport instead.
type UsbWorker = Worker<elanprint_usb::Device>;
...
impl Device {
    ...
    ///
    /// A running operation holds the worker while it waits for a finger,
    /// so locking here would hang instead of answering `AlreadyInUse`.
...
async fn main() -> anyhow::Result<()> {
    ...
    let store_path = std::env::var("ELANPRINT_STORE")
        .map(PathBuf::from)
        .unwrap_or_else(|_| PathBuf::from(elanprint_store::DEFAULT_PATH));
...
/// Suspend drops the handle, resume reopens lazily.
