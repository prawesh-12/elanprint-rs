...
//!
//! Claim held for the daemon's lifetime, armed once. A second client
//! during an operation gets `AlreadyInUse`; two sessions desync the chip.
//!
//! Erase is reachable only via [`Worker::delete_finger`] with a
...
use elanprint_proto::{Command, Enroll, EnrollAction, Response, SlotState, Status};
use elanprint_store::Store;
use elanprint_usb::{EndpointIn, Transport, UsbError};
use tokio::sync::mpsc;
...
pub enum WorkerError {
    ...
    Proto(#[from] elanprint_proto::ProtoError),
    #[error("store error: {0}")]
    Store(#[from] elanprint_store::StoreError),
    ...
    Enroll(#[from] elanprint_proto::EnrollError),
    #[error("cancelled")]
    Cancelled,
}
...
///
/// [`Worker::enroll`] and [`Worker::verify`] take it by value and close it
/// on every return path, so no operation ends without telling the client.
pub struct OpSink {
...
///
/// An operation holds the worker mutex while it waits for a finger, which
/// can be minutes. A property getter that locked it would block that long,
/// and a client reading one between `EnrollStart` and subscribing to
/// `EnrollStatus` would never subscribe.
...
///
/// Only [`OpState::try_begin`] makes one, so the flag is taken once per
/// operation and released on an early return or a panic. Taking it twice
/// once stopped every spawned op from reaching the device.
#[derive(Debug)]
...
///
/// The only key to [`Worker::delete_finger`], the only function that sends
/// an erase. It names a user and finger a human chose, so no store
/// comparison, reconcile pass or slot scan can build one.
#[derive(Debug, Clone, PartialEq, Eq)]
...
/// opened or last aborted. No touch-wait runs without it.
...
impl<T: Transport> Worker<T> {
    ...
    /// Ends the chip session first. The handle stays open, disarmed.
    ...
    /// Drops any stale handle. `claim()` reopens lazily.
    ...
    ///
    /// The [`DeleteIntent`] keeps it out of reach of a store comparison.
    /// The store says which slot the finger is in, never that it should go.
    pub async fn delete_finger(&mut self, intent: &DeleteIntent) -> Result<(), WorkerError> {
        let (user, finger) = (intent.user(), intent.finger());
        if !elanprint_store::is_valid_finger(finger) {
        ...
        if !matches!(status, Status::Ok(0)) {
            return Err(WorkerError::Enroll(elanprint_proto::EnrollError::Device {
                command: "delete",
    ...
    ///
    /// Re-arms when the flag is clear, aborts and disarms on error. Happy
    /// paths send no extra bytes.
    pub async fn verify(
        &mut self,
        _op: &OpToken,
        finger: String,
    ...
    async fn verify_inner(
        ...
        loop {
            ...
            let done = !matches!(
                ...
                Ok(elanprint_proto::VerifyOutcome::Retry(_))
            );
    ...
    ///
    /// `enrolled_num` sets the floor and is the only reading that proves a
    /// slot is in use: 0c90 answers `finger_info` with `40 ff` for occupied
    /// and empty alike. The store only rules more slots out. A 70 byte
    /// record vetoes a candidate too.
    async fn free_slot(&mut self, cancel: &CancellationToken) -> Result<u8, WorkerError> {
        let count = self.send_arm().await?;
        self.armed = true;
        let tracked = {
            let store = Store::open(&self.store_path)?;
            elanprint_store::tracked_slots(store.prints())
        };
        for id in elanprint_store::writable_slots(count, &tracked) {
...
fn endpoint(ep: elanprint_proto::ReplyEndpoint) -> EndpointIn {
    match ep {
        elanprint_proto::ReplyEndpoint::Image => EndpointIn::Image,
        elanprint_proto::ReplyEndpoint::Status => EndpointIn::Status,
        elanprint_proto::ReplyEndpoint::TouchWait => EndpointIn::TouchWait,
    }
}
