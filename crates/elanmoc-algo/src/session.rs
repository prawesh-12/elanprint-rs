...
/// Everything the engine can fail with. Never a grant path.
...
impl VerifySession {
    /// Start a login for `user` with the PAM attempt budget.
    pub fn new(user: &str) -> Self {
        Self::with_policy(user, AttemptPolicy::pam_default())
    }

    /// Same, with an explicit budget for tests.
    ...
    /// Current state, for the screen to render.
    pub fn state(&self) -> SessionState {
        self.state.clone()
    }

    /// Finger the chip selected, if it named one.
    pub fn selected(&self) -> Option<&str> {
        self.selected.as_deref()
    }

    /// A verify round started. Prompts for the first touch.
    pub fn begin(&mut self) -> Action {
        self.state = SessionState::Waiting { failures: 0 };
        Action::Prompt("touch the sensor".to_string())
    }

    /// Feed one daemon event in, get the screen action out.
