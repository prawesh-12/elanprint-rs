...
/// Failed-attempt budget. Mirrors the PAM `max_tries` value.
...
impl AttemptPolicy {
    /// Allow `max_tries` failed attempts before lockout.
    ...
    /// PAM default: three tries, then password fallback takes over.
    pub fn pam_default() -> Self {
        Self::new(3)
    }

    /// Whether another attempt is allowed after `used` failures.
    pub fn allows(&self, used: u32) -> bool {
        used < self.max_tries
    }

    /// Tries left after `used` failures, saturating at zero.
