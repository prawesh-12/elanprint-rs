...
//!
//! The mark is the whole feedback surface: a ridge lights per accepted
//! sample, a ripple fires on every chip reply, a sweep runs while waiting.
//! Someone watching can tell the reader is alive without reading a word.
...
pub struct Mark {
    ...
    pub lit: u8,
    /// Total, so any stage count still fills the mark.
    pub total: u8,
    /// Unlit ridge colour.
    ...
    /// Scan band position, 0 to 1, top to bottom.
