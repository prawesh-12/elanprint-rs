...
impl Enroll {
    ...
    ///
    /// The machine knows what it queued, so callers never guess from the
    /// outgoing bytes. Reading the collision check on a sample's endpoint
    /// costs a full touch timeout and no commit.
    ...
    /// Returns the first sample command.
